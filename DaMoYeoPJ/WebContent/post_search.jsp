	...
	<script type="text/javascript">
		...
		window.onload=function(){
			...
			for(var i=0; i < size/10; i++){
				taga.innerHTML += " <a href='controller.do?command=searchResult&searchtext="+searchtext+"&sort="+sort+"&page="+ (i+1) +"')>"+ (i+1)+ "</a> ";
			}
		};
...
%>
					...
					<input type="hidden" value="<%=searchtext %>" id="text" />
				</form>
				...
				<div id="paging">
					<%
						if(pagecnt != 1){
					%>
					<a href="controller.do?command=searchResult&searchtext=<%=searchtext %>&sort=<%=sort %>&page=<%=pagecnt+1 %>" id="pre">이전</a>
					<%
						}
						else{
					%>
					...
					%>
					<a href="controller.do?command=searchResult&searchtext=<%=searchtext %>&sort=<%=sort %>&page=<%=pagecnt-1 %>" id="next">다음</a>
					<%
